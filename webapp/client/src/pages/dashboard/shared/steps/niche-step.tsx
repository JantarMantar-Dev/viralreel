...
import CustomNicheForm from "../../create/components/custom-niche-form"

import { API_BASE_URL } from "@/lib/config"

export default function SharedNicheStep() {
    const { nicheId, updateNicheId, jobType } = useSharedCreation()
    ...
    if (showCustomForm) {
        return <CustomNicheForm onBack={() => setShowCustomForm(false)} />
    }

    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <StepHeader
                title={jobType === "series" ? "What's your series about?" : "What's your video about?"}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                {isLoading ? (
                    ...
                    <div className="col-span-full py-12 text-center">
                        ...
                        <button
                            onClick={() => queryClient.invalidateQueries({ queryKey: ["niches"] })}
                    ...
                    <>
                        {niches?.map((niche) => {
                            ...
                            return (
                                <div
                                    key={niche.id}
                                    onClick={() => updateNicheId(niche.id, niche.name)}
                                    className={cn(
                        ...
                        <div
                            className="group relative p-6 md:p-8 rounded-2xl md:rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50/50 hover:bg-white hover:border-purple-300 hover:border-solid transition-all duration-300 cursor-pointer flex flex-col focus:ring-2 focus:ring-purple-500 min-h-[220px]"
                            onClick={() => {
                                updateNicheId(null, "")
                        ...
                        >
                            ...
                            <div className="mt-auto inline-flex items-center text-purple-600 font-bold text-sm">
                                Get Started <Plus className="ml-1 h-4 w-4" />
                            </div>
                        </div>
                    </>
                )}
