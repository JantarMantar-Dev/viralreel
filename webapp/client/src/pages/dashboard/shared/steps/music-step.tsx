...
import { useSharedCreation } from "../context/shared-creation-interface"
import { Button } from "@/components/ui/button"
import StepHeader from "../../create/components/step-header"
import AudioList from "../../create/components/audio-list"
import UploadMusicDialog from "../../create/components/upload-music-dialog"
import DeleteMusicDialog from "../../create/components/delete-music-dialog"
...
export default function SharedMusicStep() {
    const { musicId, musicName, updateMusicId, jobType, scriptIdea } = useSharedCreation()
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (musicId === trackToDelete) {
                updateMusicId(undefined, undefined, undefined)
            }
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto space-y-8">
            <StepHeader
                title="Set the tone"
                description={`Choose a background track that matches the mood of your ${jobType === "series" ? "series" : "video"} "${scriptIdea.slice(0, 30)}${scriptIdea.length > 30 ? '...' : ''}".`}
            />
            ...
            {/* Skip Option */}
            <div
                onClick={() => updateMusicId(undefined, undefined, undefined)}
                className={cn(
            ...
            >
                <div className="flex items-center gap-4">
                    <div className={cn(
                        ...
                        !musicId ? "bg-purple-600 text-white" : "bg-slate-100 text-slate-400 group-hover:text-purple-600"
                    )}>
                        <Music className="h-6 w-6" />
                    </div>
                    <div>
                        <h4 className={cn("font-bold text-base", !musicId ? "text-purple-900" : "text-slate-900")}>Skip Background Music</h4>
                        <p className="text-xs font-semibold text-slate-400">Create the {jobType === "series" ? "series" : "video"} without any background track</p>
                    </div>
                </div>
                {!musicId && (
                    ...
                    onSelect={(id) => {
                        ...
                        if (track) {
                            const details = track.createdAt
                                ? `Added ${new Date(track.createdAt).toLocaleDateString()}`
                                : "Default Music"
                            updateMusicId(id, track.name, details)
                        }
                    }}
